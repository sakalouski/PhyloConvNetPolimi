...
while(gene_interator < X.shape[1]-num_of_genes):
    ...
    out_scores, out_ranks, out_inds = pcnp.cluster_generator_new(tcga_X=X_train,tcga_Y=Y_train,
                                                                 num_iters=100,
                                                                 subset_size=50,
                                                                 genes_to_take=kern_size,
                                                                 num_of_top_to_choose=20)
    dm = pcnp.get_distance_matrix(out_scores, out_ranks, out_inds, num_of_genes)
...
df_report.to_csv('report_lung_subtypes_performance_based.csv')