...
for i in filtered_df.index.tolist():
    if (filtered_df.tumor_type.loc[i] == 'LUNG'
        or filtered_df.tumor_type.loc[i] == 'LUSC'
...
for i in range(400):
    ...
    for i in range(d_m.shape[0]):
        temp = np.argsort(d_m[i, :])[1:7]
        if temp.all() < 100:
            indexes_all.append(i)

    rand_inds = np.random.randint(high=len(indexes_all), low=0, size=300).tolist()
    indexes = (np.array(indexes_all)[rand_inds]).tolist()
