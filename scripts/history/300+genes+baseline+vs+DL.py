...
def svm_test(X_train, Y_train, X_test, Y_test):
    ...
    score_svm = f1_score(Y_test, y_pred, average='weighted')
    return score_svm
...
# ### Getting the baseline
    score_svm = svm_test(X_train, Y_train.astype(str), X_test, Y_test.astype(str))
...
# ### Preparing data for DL
    ...
    #print(order)
    X_dl = X_300[:,order]
    score_dl = dl_test(X_dl,Y_temp)
    scores.append([score_dl, score_svm])
    if (score_dl>score_svm):
        winner_dl += 1
    else:
        winner_svm += 1
    print('DL won vs. SVM won: ', winner_dl, '/', winner_svm)
